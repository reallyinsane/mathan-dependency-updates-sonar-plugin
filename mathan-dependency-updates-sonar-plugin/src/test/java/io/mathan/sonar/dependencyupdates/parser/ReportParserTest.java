...
public class ReportParserTest {
  ...
  public void parseReport() throws Exception {
    ...
    verifyPresent(
        ...
        Collections.emptyList(), Arrays.asList("4.1.0", "4.1.1", "4.1.2"), Collections.emptyList(), Availablility.Minor);
    verifyPresent(
        dependencyManagements, "dependencymanagement-with-major", null, null, "pom", "1.4.0", "2.0.0",
        Collections.emptyList(), Collections.emptyList(), Arrays.asList("2.0.0", "2.1.0"), Availablility.Major);
    verifyPresent(
        dependencyManagements, "dependencymanagement-with-incremental", null, null, "pom", "4.2.2", "4.2.3",
        Arrays.asList("4.2.3"), Collections.emptyList(), Collections.emptyList(), Availablility.Incremental);
    verifyPresent(
        dependencyManagements, "dependencymanagement-with-none", null, null, "pom", "3.0.9", null,
        Collections.emptyList(), Collections.emptyList(), Collections.emptyList(), Availablility.None);
  ...
  private static void verifyPresent(
      ...
      String version, String next, List<String> incrementals, List<String> minors, List<String> majors, Availablility availablility) {
    Dependency check = new Dependency();
    check.setArtifactId(artifactId);
    check.setAvailablility(availablility);
